...
the spec JSON and `.g2g-goal`. `.g2g-goal` is ephemeral — it must never be
committed, and you must delete it before the build reaches any terminal
...
1. Before anything else: if a `.g2g-goal` file already exists in the repo
   ...
   check in step 2 to catch this incidentally — `.g2g-goal` is untracked,
...
5. Rebase onto the default branch. Conflicts: STOP — delete `.g2g-goal`
   ...
   --abort`, then push and open a draft PR titled "g2g: <project>
...
6. Clean rebase: push ONCE (`git push -u origin <branch>`), then
   ...
   trailers). NEVER merge. Delete `.g2g-goal` now that the build has
